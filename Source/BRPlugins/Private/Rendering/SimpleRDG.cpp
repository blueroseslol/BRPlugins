...
namespace SimpleRenderingExample
{
	...
	void RDGCompute(FRHICommandListImmediate &RHIImmCmdList, FTexture2DRHIRef RenderTargetRHI, FSimpleShaderParameter InParameter)
	{
		...
		FPooledRenderTargetDesc RenderTargetDesc = FPooledRenderTargetDesc::Create2DDesc(RenderTargetRHI->GetSizeXY(),
																						 RenderTargetRHI->GetFormat(), FClearValueBinding::Black, TexCreate_None, TexCreate_RenderTargetable | TexCreate_ShaderResource | TexCreate_UAV, false);
		TRefCountPtr<IPooledRenderTarget> PooledRenderTarget;
		//GRenderTargetPool.CreateUntrackedElement(RenderTargetDesc, PooledRenderTarget, RenderTargetItem);
		GRenderTargetPool.FindFreeElement(RHIImmCmdList, RenderTargetDesc, PooledRenderTarget, TEXT("ComputeRenderTarget"));

		//RDG Begin
		FRDGBuilder GraphBuilder(RHIImmCmdList);
		FRDGTextureRef RDGRenderTarget = GraphBuilder.RegisterExternalTexture(PooledRenderTarget, TEXT("ComputeRenderTarget"));
	...
	void RDGDraw(FRHICommandListImmediate &RHIImmCmdList, FTexture2DRHIRef RenderTargetRHI, FSimpleShaderParameter InParameter, const FLinearColor InColor, FTexture2DRHIRef InTexture)
	{
		...
		TRefCountPtr<IPooledRenderTarget> PooledRenderTarget;
		//GRenderTargetPool.CreateUntrackedElement(RenderTargetDesc, PooledRenderTarget, RenderTargetItem);
		GRenderTargetPool.FindFreeElement(RHIImmCmdList, RenderTargetDesc, PooledRenderTarget, TEXT("DrawRenderTarget"));

		//RDG Begin
		FRDGBuilder GraphBuilder(RHIImmCmdList);
		FRDGTextureRef RDGRenderTarget = GraphBuilder.RegisterExternalTexture(PooledRenderTarget, TEXT("DrawRenderTarget"));
